      ...
      {rows.length === 0 ? (
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead>
              <tr>
                {['Group', 'Distinct appointments', 'Units', 'Pallets'].map((heading) => (
                  <th key={heading} className="px-3 py-2 text-left font-semibold text-gray-800">{heading}</th>
                ))}
              </tr>
            </thead>
...
export function ReportingPage() {
  ...
  const exportXlsx = () => {
    if (!validation.valid || rows.length === 0) return;
    const workbookBytes = buildXlsxExport({
      ...
      <section className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200" aria-labelledby="report-controls-title">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="text-sm font-medium text-gray-700">
            Preset anchor date
            <input aria-label="Preset anchor date" type="date" value={anchorDate} onChange={(event) => setAnchorDate(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
          </label>
          ...
          <label className="text-sm font-medium text-gray-700">
            Report mode
            <input aria-label="Report mode" readOnly value={sanitized.mode} className="mt-1 w-full rounded-md border border-gray-300 bg-gray-50 px-3 py-2" />
          </label>
          <label className="text-sm font-medium text-gray-700">
            Report level
            <select aria-label="Report level" value={sanitized.level} onChange={(event) => changeLevel(event.target.value as ReportLevel)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="PO">PO — one row per appointment</option>
              <option value="SKU">SKU — one row per product line</option>
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Inclusive date from
            <input aria-label="Inclusive date from" type="date" value={sanitized.dateFrom} onChange={(event) => updateDate('dateFrom', event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
          </label>
          <label className="text-sm font-medium text-gray-700">
            Inclusive date to
            <input aria-label="Inclusive date to" type="date" value={sanitized.dateTo} onChange={(event) => updateDate('dateTo', event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
          </label>
          <label className="text-sm font-medium text-gray-700">
            Warehouse
            <select aria-label="Report warehouse" value={sanitized.filters.warehouseId} onChange={(event) => setDefinition(setFilter(sanitized, 'warehouseId', event.target.value as ReportFilters['warehouseId']))} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="all">All scoped warehouses</option>
              {warehouseOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Supplier
            <select aria-label="Report supplier" value={sanitized.filters.supplierOrganizationId} onChange={(event) => setDefinition(setFilter(sanitized, 'supplierOrganizationId', event.target.value as ReportFilters['supplierOrganizationId']))} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="all">All scoped suppliers</option>
              {supplierOptions.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Booking origin
            <select aria-label="Report booking origin" value={sanitized.filters.bookingOrigin} onChange={(event) => setDefinition(setFilter(sanitized, 'bookingOrigin', event.target.value as ReportFilters['bookingOrigin']))} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="all">All origins</option>
              <option value="SUPPLIER_RESERVED">SUPPLIER_RESERVED</option>
              <option value="ADMIN_ADDED">ADMIN_ADDED</option>
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Planning state
            <select aria-label="Report planning state" value={sanitized.filters.planningState} onChange={(event) => setDefinition(setFilter(sanitized, 'planningState', event.target.value as ReportFilters['planningState']))} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="all">All planning states</option>
              {['AWAITING_DETAILS','DETAILS_ATTACHED','VALIDATION_CONFLICT','READY'].map((state) => <option key={state} value={state}>{state}</option>)}
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Appointment status
            <select aria-label="Report appointment status" value={sanitized.filters.appointmentStatus} onChange={(event) => setDefinition(setFilter(sanitized, 'appointmentStatus', event.target.value as ReportFilters['appointmentStatus']))} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              <option value="all">All appointment statuses</option>
              {['DRAFT','SUBMITTED','PENDING_APPROVAL','CONFIRMED','REJECTED','CANCELLED'].map((status) => <option key={status} value={status}>{status}</option>)}
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700 xl:col-span-2">
            Report search
            <input aria-label="Report search" type="search" value={sanitized.filters.search} onChange={(event) => setDefinition(setFilter(sanitized, 'search', event.target.value))} placeholder="Appointment, PO, SKU, description, Supplier or registration" className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
          </label>
          <label className="text-sm font-medium text-gray-700">
            Sort column
            <select aria-label="Report sort column" value={sanitized.sort.column} onChange={(event) => updateSort('column', event.target.value as ReportColumnId)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
              {columnCatalog.map((column) => <option key={column} value={column}>{reportColumnLabels[column]}</option>)}
            </select>
          </label>
          <label className="text-sm font-medium text-gray-700">
            Sort direction
            <select aria-label="Report sort direction" value={sanitized.sort.direction} onChange={(event) => updateSort('direction', event.target.value as ReportSortDirection)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2">
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            {columnCatalog.map((column) => (
              <label key={column} className="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" aria-label={`Report column ${reportColumnLabels[column]}`} checked={sanitized.columns.includes(column)} onChange={() => toggleColumn(column)} />
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={exportCsv} disabled={exportsDisabled} className="inline-flex items-center gap-2 rounded-md border border-[#023466] px-4 py-2 text-sm font-semibold text-[#023466] disabled:cursor-not-allowed disabled:opacity-50">
              <Download className="h-4 w-4" aria-hidden="true" />Export active CSV
            </button>
            <button type="button" onClick={exportXlsx} disabled={exportsDisabled} className="inline-flex items-center gap-2 rounded-md bg-[#023466] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
              <FileSpreadsheet className="h-4 w-4" aria-hidden="true" />Export active XLSX
            </button>
          </div>
        </div>
      </section>
        ...
        {rows.length === 0 ? (
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead>
                <tr>
                  {sanitized.columns.map((column) => <th key={column} scope="col" className="px-3 py-2 text-left font-semibold text-gray-800">{reportColumnLabels[column]}</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {rows.map((row) => (
                  <tr key={row.key}>
                    {sanitized.columns.map((column) => <td key={column} className="px-3 py-2 text-gray-700">{formatCell(reportCellValue(row, column))}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
