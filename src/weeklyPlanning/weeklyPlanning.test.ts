...
describe('weekly planning domain', () => {
  ...
  it('blocks transport conflicts until an authorized reasoned decision retains Supplier values', () => {
    ...
    const resolved = resolveTransportConflictKeepingSupplierValues(
      initial, evidence[0].fingerprint, 'warehouse-admin', 'Keep Supplier authority', true,
    );
  ...
  it('keeps no-match evidence unscheduled until compatible-slot authorization succeeds and uses typed transport', () => {
    ...
    expect(created.tractorRegistration).toBe('TR-TYPED');
    expect(created.trailerOrContainerRegistration).toBe('TRL-TYPED');
    expect(scheduleUnreservedDelivery(
      scheduled.state, evidence[0].fingerprint, 'warehouse-admin', 'Again', true,
      '2026-08-13', '09:00', initialDemoConfiguration.warehouses,
    ).error).toContain('not in the unscheduled queue');
  });
  ...
  it('requires explicit exact-candidate selection and records candidates, target and resulting state', () => {
    ...
    expect(resolveAmbiguousTarget(initial, evidence[0].fingerprint, 'not-a-candidate', 'admin', 'Choose target', true).error)
      .toContain('not one of the exact candidates');
