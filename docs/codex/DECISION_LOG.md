...
| DCF-016 | ACCEPTED | Remediate the React Router runtime audit finding in isolated Class C task `SPR-SEC-2`. | The preserved `AUTONOMY-GOV-1` implementation cannot pass its mandatory runtime audit, while stale post-merge roadmap state creates a bootstrap deadlock. | The runner remains preserved, paused, and blocked until remediation passes. Audit suppression, overrides, `npm audit fix`, force, and legacy-peer behavior are prohibited. Lucide React may move only from 0.394.0 to 0.397.0 to satisfy the approved React 19 peer contract. The one-time recovery amendment resolves only this stale-roadmap deadlock and creates no precedent for bypassing `READY`. |
